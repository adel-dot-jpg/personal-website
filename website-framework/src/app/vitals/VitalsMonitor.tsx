    ...
    <div className="mx-auto w-full max-w-6xl" style={{ fontFamily: "Courier New" }}>
      <div className="rounded-3xl border-[16px] border-zinc-800 bg-zinc-900 p-4 shadow-2xl md:p-8">
        <div className="relative overflow-hidden rounded-xl border-8 border-black bg-black shadow-[inset_0_0_50px_rgba(0,0,0,1)] transition-colors duration-500"
          style={{ boxShadow: powerOn ? `inset 0 0 100px ${darkTheme}` : 'none' }}>
        ...
        <div className="mt-6 flex justify-end gap-4">
          <button
            onClick={() => {
              const next = !uplinkRef.current;
              uplinkRef.current = next;
              setUplinkActive(next);
              if (!next) {
                setIsCritical(true);
                setDisplay(CRITICAL_DISPLAY);
              }
  ...
  return (
    ...
    >
      ...
      <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" className="w-full" style={{ height: '90px' }}>
        {[0.25, 0.5, 0.75].map((t) => (
          <line key={t} x1={pad} y1={H - pad - t * (H - pad * 2)} x2={W - pad} y2={H - pad - t * (H - pad * 2)} stroke={darkTheme} strokeWidth="0.5" />
        ))}
        <polyline points={recvPoints} fill="none" stroke="#3b8eea" strokeWidth="1.5" strokeLinejoin="round" strokeLinecap="round" />
...
function VitalBox({ label, value, sub, darkTheme, themeColor }: {
  label: string;
  value: string;
  sub: string;
  darkTheme: string;
